...
// #define BLOCK_BITMAP(bitmap) ((bitmap) |= ((1 << (NUM_BLOCKS)) - 1))
// #define INODE_BITMAP(bitmap) ((bitmap) |= ((1 << (NUM_INODES)) - 1))
...
void write_block_bitmap(int fd) {
    ...
    for (int i = 0; i < LAST_BLOCK; i++) {
        // Set the bit at index i to 1 (block i is used)
        bitmap[i / 8] |= 1 << (i % 8);
    }
    for (int i = 1023; i < 8193; i++) {
        // Set the bit at index i to 1 (inode i is used)
        bitmap[i / 8] |= 1 << (i % 8);
    }
...
void write_inode_table(int fd) {
    ...
    const char* hello_inode_target = "hello-world";
	...
	hello_inode.i_size = strlen(hello_inode_target);
    ...
    strcpy((char *)&hello_inode.i_block, hello_inode_target);
